...
def test_single_char_token_matches_exact_rarity_only(client):
    ...
    with patch("services.card_service.fetch_ebay_prices", return_value={"eBay": 100.0}):
...
def test_sec_rarity_card_found(client):
    with patch("services.card_service.fetch_ebay_prices", return_value={"eBay": 150.0}):
...
def test_suggestion_uses_sheet_prices_when_ebay_unavailable(client, reset_limiter):
    with patch("services.suggestion_service.fetch_ebay_prices", return_value=None), \
